...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const unresolvedHigh = 0;

  const { selectedHouse, setSelectedHouse, selectedTerm, setSelectedTerm } = useData();
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-2.5 sm:px-6">
            ...
            <div className="flex shrink-0 items-center gap-4">
              <div className="flex items-center rounded-md border border-border bg-card p-0.5 text-sm">
                <button
                  ...
                  className={cn(
                    ...
                    selectedHouse === "Lok Sabha" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"
                  )}
                >
                  Lok Sabha
                </button>
                <button
                  ...
                  className={cn(
                    "px-3 py-1.5 rounded-sm font-medium transition-colors",
                    selectedHouse === "Rajya Sabha" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"
                  )}
                >
                  Rajya Sabha
                </button>
              ...
              {selectedHouse === "Lok Sabha" && (
                <div className="flex items-center rounded-md border border-border bg-card p-0.5 text-sm">
                  <button
                    onClick={() => setSelectedTerm("18")}
                    className={cn(
                      "px-3 py-1.5 rounded-sm font-medium transition-colors",
                      selectedTerm === "18" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"
                    )}
                  >
                    18th Term
                  </button>
                  <button
                    onClick={() => setSelectedTerm("17")}
                    className={cn(
                      "px-3 py-1.5 rounded-sm font-medium transition-colors",
                      selectedTerm === "17" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent"
                    )}
                  >
                    17th Term
                  </button>
